...
const IncomeManagement = () => {
  ...
  const calculateTotalIncome = () => {
    return incomeData.reduce((sum, item) => sum + (Number(item.totalIncome) || 0), 0);
  };

  const calculateTotalOrders = () => {
    return incomeData.reduce((sum, item) => sum + (Number(item.totalOrders) || 0), 0);
  };
  ...
  const processTopTours = (data) => {
    ...
    const sortedData = [...data].sort((a, b) => b.totalIncome - a.totalIncome);
    ...
    const otherIncome = sortedData.slice(10).reduce((sum, item) => sum + item.totalIncome, 0);
  ...
  return (
    <div className="p-6">
      ...
      <Row gutter={16} className="mb-6">
        ...
        <Col span={8}>
          <Card>
            <Statistic
              title="Trung Bình/Vé"
              value={calculateTotalOrders() ? calculateTotalIncome() / calculateTotalOrders() : 0}
              formatter={(value) => formatCurrency(value)}
            />
          </Card>
        </Col>
      ...
      <Row gutter={16} className="mb-6">
        ...
        <Col span={12}>
          <Card title="Phân bố Thu nhập theo Tour" className="text-center">
            {tourData.length > 0 ? (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <PieChart width={500} height={400}>
                  <Pie
                    ...
                    outerRadius={150}
                    fill="#8884d8"
                    label={(entry) => `${truncateText(entry.TENTOUR)}: ${formatCurrency(entry.totalIncome)}`}
                  >
                    {processTopTours(tourData).map((entry, index) => (
